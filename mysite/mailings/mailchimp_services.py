...
def add_email_with_tag(audience_name: str, email: str, tag: str) -> None:
    """Добавляет в mailchimpemail в аудиторию с названием audience_name"""
    _add_email_to_mailchimp_audience(audience_id=settings.MAILCHIMP_AUDIENCES.get(audience_name), email=email)
    _add_mailchimp_tag(audience_id = settings.MAILCHIMP_LIST_ID, subscriber_hash = _get_mailchimp_subscriber_hash(email), tag = tag) 
...
def _get_mailchimp_subscriber_hash(email:str) -> Optional[str]:
    """Возвращает идентификатор email в mailchimp или None, если email не найден"""
...
def _add_mailchimp_tag(audience_id: str, subscriber_hash: str, tag: str) -> None:
    """Добавляет тег для email с идентификатором subscriber_hash в аудитории audience_id"""
